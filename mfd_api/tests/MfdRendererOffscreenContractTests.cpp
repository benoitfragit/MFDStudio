...
TEST(MfdRendererOffscreenContractTests, RequestDefaultsToInvalidDimensions)
{
    ...
    EXPECT_FALSE(request.HasValidDimensions());
}

TEST(MfdRendererOffscreenContractTests, RequestDimensionValidationMatchesContract)
{
    mfd::OffscreenRenderRequest request {};
    request.width = 128;
    request.height = 0;
    EXPECT_FALSE(request.HasValidDimensions());

    request.height = 256;
    EXPECT_TRUE(request.HasValidDimensions());
}
...
TEST(MfdRendererOffscreenContractTests, DefaultResultCarriesUnavailableMessage)
{
    ...
    EXPECT_EQ(result.message, "render target unavailable");
}
} // namespace
