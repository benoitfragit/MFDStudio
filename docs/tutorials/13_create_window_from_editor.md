...
This also matters for the integrated tutorial: `client_tutorial` is intentionally kept out of the default top-level build. Once the authored tutorial window, page, and reticle JSON files exist under the repository `assets/` tree, add `mfd_add_repo_subdirectory(examples/client_tutorial)` to the root `CMakeLists.txt`, re-run CMake configuration, and then build the new target. `Start-MfdTutorial.bat` becomes usable at the same point.
