...
/**
 * @file
 * @brief Implementation of the Dear ImGui Demo HUD demo application.
 */
...
void ApplyStickPovInput(PilotControls& controls, DemoManeuver& maneuver, const ImVec2 center, const float radius) noexcept
{
    ...
    maneuver = DemoManeuver::Manual;
}

void DrawStickPov(PilotControls& controls, DemoManeuver& maneuver)
{
...
DemoHudApplication::DemoHudApplication()
{
...
bool DemoHudApplication::Initialize(std::string& error)
{
    DemoConfig loadedConfig;
    ...
    SetStatus("Demo HUD demo connected.", false);
    return true;
}

bool DemoHudApplication::LoadConfig(DemoConfig& config, std::string& error) const
{
    try
    {
        mfd::JsonLoader loader;
        // The generated UI class owns the canonical window-json path. Reading
        // it here keeps the demo client synchronized with regenerated assets.
        const mfd::LoadedWindowConfiguration loaded =
            loader.LoadWindowConfiguration(std::string(demo_hud_ui::DemoHudUi::WindowFile()));

        const mfd::PageDefinition* hudPage =
            mfd::FindPageDefinition(loaded.document, demo_hud_ui::HUDMockupPage::Name());
        if (hudPage == nullptr)
        {
            error = "The Demo HUD asset does not expose a page named 'HUD'.";
            return false;
        }
        ...
        if (!loaded.window.commandTransports.udp.has_value() || !loaded.window.commandTransports.udp->enabled)
        {
            ...
            // this transport the demo cannot publish any visible frame.
            error = "The Demo HUD window JSON has no enabled UDP command transport.";
            return false;
        }
        ...
        if (!loaded.generatedTransportMap.has_value())
        {
            ...
            error = "The Demo HUD generated transport map is missing.";
            return false;
        }
...
bool DemoHudApplication::Connect(const DemoConfig& config, std::string& error)
{
    ...
    if (!ui_.SendStartup(*startupClient_, config.pageView, "Demo HUD DEMO | READY"))
    {
...
void DemoHudApplication::DrawFrame(const float deltaSeconds)
{
...
void DemoHudApplication::UpdateHudInputBufferFromUi(const float deltaSeconds) noexcept
{
    // This is the demo-client adapter boundary. In a real integration, replace
    ...
    // They must first become this semantic SI buffer, then `DemoHudController`
...
void DemoHudApplication::Shutdown()
{
    if (publisher_ != nullptr && publisher_->IsReady())
    {
        // The final caption helps distinguish a clean client shutdown from a
        // lost feedback/transport failure in the HUD window.
        ui_.SubmitShutdown(*publisher_, sequence_++, "Demo HUD DEMO | CLIENT STOPPED");
        publisher_->Flush();
    }
}

void DemoHudApplication::ApplyKeyboardControls()
{
    ...
    if (pitchCommand != 0 || rollCommand != 0)
    {
        ...
        maneuver_ = DemoManeuver::Manual;
    }
}

void DemoHudApplication::SelectManeuver(const DemoManeuver maneuver) noexcept
{
    maneuver_ = maneuver;
    if (maneuver_ == DemoManeuver::Manual)
    {
...
void DemoHudApplication::ApplyManeuverControls() noexcept
{
    // Scripted maneuvers write the same control fields as manual input; there
    // is no alternate path into the HUD or simulation.
    switch (maneuver_)
    {
    case DemoManeuver::Loop:
        controls_.pitchCommand = 1.0f;
        controls_.rollCommand = 0.0f;
        break;
    case DemoManeuver::BarrelRoll:
        controls_.pitchCommand = 0.35f;
        controls_.rollCommand = 1.0f;
        break;
    case DemoManeuver::Manual:
        break;
    }
}

void DemoHudApplication::SyncHudInputBufferFromSimulation() noexcept
{
    // Keep the buffer copy explicit. This makes the demo boundary obvious and
    // leaves room for a future external-aircraft producer to replace it.
    hudInputs_ = simulation_.Inputs();
}

void DemoHudApplication::DrawConnectionPanel()
{
    ...
    if (ImGui::BeginTable("##connection_header", 3, ImGuiTableFlags_SizingStretchProp))
    {
        ...
        ImGui::TextUnformatted("DEMO HUD CONTROL PANEL");
    ...
    if (DrawPanelButton(
            ...
            "Start or pause the demo aircraft simulation."))
    {
        running_ = !running_;
    }
            ...
            "Reconnect the UDP publisher and resend the generated HUD startup state."))
    {
        ...
        {
            SetStatus("Demo HUD demo reconnected.", false);
        }
    }
    ImGui::Separator();
}

void DemoHudApplication::DrawFlightControls()
{
    if (BeginControlPanel("Aircraft"))
    {
        ...
        DrawToggleButton(
            ...
            ImVec2(kActionButtonWidth, kActionButtonHeight),
            "Request afterburner in the demo aircraft model.");
    }
    ...
    if (BeginControlPanel("Maneuver"))
    {
        ...
        {
            SelectManeuver(DemoManeuver::Manual);
        }
        ImGui::SameLine();
        if (DrawPanelButton(
                "LOOP",
                maneuver_ == DemoManeuver::Loop,
                ImVec2(kActionButtonWidth, kActionButtonHeight),
                "Command a scripted pitch-up loop."))
        {
            SelectManeuver(DemoManeuver::Loop);
        }
        ImGui::SameLine();
        if (DrawPanelButton(
                "BARREL",
                maneuver_ == DemoManeuver::BarrelRoll,
                ImVec2(kActionButtonWidth, kActionButtonHeight),
                "Command a scripted barrel-roll maneuver."))
        {
            SelectManeuver(DemoManeuver::BarrelRoll);
        }
    }
...
void DemoHudApplication::DrawWeaponControls()
{
    ...
    if (BeginControlPanel("Missile"))
    {
        ...
        if (DrawPanelButton(
                ...
                false,
                ImVec2(kActionButtonWidth, kActionButtonHeight),
                "Cycle the selected missile type in the demo inventory."))
        {
    ...
    if (BeginControlPanel("Landing / ILS"))
    {
        ...
        if (DrawToggleButton(
                ...
                ilsSelected,
                ImVec2(kToggleButtonWidth, kToggleButtonHeight),
                "Power and select the demo ILS receiver."))
        {
            SetIlsDemoEnabled(ilsSelected);
        }
        ImGui::SameLine();
        DrawToggleButton(
            "ILS CMD",
...
void DemoHudApplication::DrawTelemetryPanel()
{
...
void DemoHudApplication::PublishFrame()
{
...
void DemoHudApplication::PollFeedback()
{
...
void DemoHudApplication::ResetScene()
{
...
void DemoHudApplication::FireSelectedMissile()
{
...
void DemoHudApplication::SelectMissile(const MissileType missileType) noexcept
{
...
void DemoHudApplication::SelectHudMode(const HudMasterMode masterMode,
...
void DemoHudApplication::SetIlsDemoEnabled(const bool enabled) noexcept
{
...
void DemoHudApplication::SetStatus(std::string status, const bool error)
{
