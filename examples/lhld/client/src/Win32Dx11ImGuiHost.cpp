...
/**
 * @file
 * @brief Implementation of the private Win32 + DX11 host used by `lhld_client`.
 */
...
std::wstring ToWideString(const std::string& value)
{
    ...
    // MultiByteToWideChar with cbMultiByte = -1 writes the terminating NUL, so the
    // destination must hold all `requiredSize` code units. The NUL is then trimmed
    // from the returned string instead of being written past the buffer end.
    std::wstring wide(static_cast<std::size_t>(requiredSize), L'\0');
    const int written = MultiByteToWideChar(CP_UTF8, 0, value.c_str(), -1, wide.data(), requiredSize);
    if (written <= 0)
    {
        return std::wstring(value.begin(), value.end());
    }
...
struct Win32Dx11ImGuiHost::Impl
{
    ...
    float clearColor[4] {6.0f / 255.0f, 10.0f / 255.0f, 14.0f / 255.0f, 1.0f};

    static constexpr const wchar_t* kWindowClassName = L"MfdClientLhldWin32Dx11Host";
...
ID3D11Device* Win32Dx11ImGuiHost::Device() const noexcept
{
#if defined(_WIN32)
...
ID3D11DeviceContext* Win32Dx11ImGuiHost::DeviceContext() const noexcept
{
#if defined(_WIN32)
    return impl_ == nullptr ? nullptr : impl_->deviceContext;
#else
    return nullptr;
#endif
}
